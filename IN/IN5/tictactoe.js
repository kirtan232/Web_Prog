...
// elements are the top row, the next 3 the middle row, and the last 3 the bottom row.
...
function newGame() {   // TODO: Complete the function
...
function boardButtonClicked(button) {   // TODO: Complete the function
    if (!playerTurn || button.disabled) return;
...
function switchTurn() {   // TODO: Complete the function
    ...
    if (status === gameStatus.MORE_MOVES_LEFT) {
        ...
        playerTurn = false;
        if (status === gameStatus.HUMAN_WINS) info.textContent = "You win!";
        else if (status === gameStatus.COMPUTER_WINS) info.textContent = "Computer wins!";
        else info.textContent = "Draw game";
    }
}

function makeComputerMove() {   // TODO: Complete the function
    const buttons = getGameBoardButtons();
